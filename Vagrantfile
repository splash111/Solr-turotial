...
###end
Vagrant.configure("2") do |config|
  #provision use inline script
  config.vm.provision :shell, inline: "/bin/bash /vagrant/scripts/provision.sh"
  # Defines the Vagrant box name, download URL, IP and hostname
  config.vm.box = "parallels/ubuntu-12.04"
  # Set The provider and check for tools update and name for Parallels Desktop
config.vm.provider "parallels" do |v|
  v.update_guest_tools = true
  v.name = "Solr Tutorial"
end
  config.vm.define :vagrant do |vagrant|
    ...
    vagrant.vm.network "forwarded_port", guest: 8983, host: 8899
